...
function inlineModules(filePath, visited = new Set()) {
    ...
    const strippedSource = source.replaceAll(importRegex, "");

    const parts = specifiers.map((specifier) => inlineModules(path.resolve(dir, specifier), visited));

    const exportStripped = strippedSource
        .replaceAll(/^export\s+default\s+(function|class)(\s)/gm, "$1$2")
        .replaceAll(/^export\s+default\s+/gm, "var _moduleDefault = ")
        .replaceAll(/^export\s+(function|class|var|let|const)\s/gm, "$1 ")
        .replaceAll(/^export\s*\{[^}]*\}(?:\s+from\s+["'][^"']*["'])?;?[ \t]*(?:\r?\n|$)/gm, "");
        ...
        for (const decl of node.declarations) {
            if (decl.id.name === "PluginParameters" && decl.init?.type === "ArrayExpression") {
                return decl.init.elements.map((el) => {
                    if (el?.type !== "ObjectExpression") return 0;
...
/**
 * Builds a footer comment for the compiled PST file, including the compiler version.
 *
 * @returns {string} Footer comment
 */
function buildFooterComment() {
    const INNER = 41; // chars between "/* " and "*/" (line width 46 minus 3 + 2)
    const sep = "/" + "*".repeat(44) + "/";
    ...
    return (
        ...
        sep + "\n" +
        line("Compiled with:") + "\n" +
        line("@benjamindehli/logic-pro-script-compiler") + "\n" +
        line("v" + version) + "\n" +
        sep
    );
}
...
function compileToPst(jsSource, filePath = null) {
    ...
    const jsBytes = Buffer.from(transpile(sourceToCompile).trimEnd() + "\n" + buildFooterComment(), "utf8");
