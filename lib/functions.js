...
function extractParamDefaults(jsSource) {
    try {
        const ast = parse(jsSource, { sourceType: "script", errorRecovery: true });
...
function compileToPst(jsSource) {
    const jsBytes = Buffer.from(jsSource, "utf8");
    ...
    // Write parameter default values as float32 LE (same format Logic Pro uses)
